...
features can never be subtracted, a limitation recently criticized in the
signed-hyper-connections (sHC) line of work. We propose \textbf{Uni-mHC}, which
...
four-stream topology---instantiated here with Uni-mHC---reaches $2.3\times$ lower
validation loss than an identically trained vanilla GPT.
\end{abstract}
...
\citep{lu2025hyperconnections}, which maintain $n$ parallel streams with a learned
...
coupling pushes deep stacks toward identity degeneration; yet simply allowing
negative entries abandons the energy-conservation guarantees that motivated mHC.
...
\end{tikzpicture}
\caption{\textbf{Uni-mHC operator.} The complex Cayley transform of a
skew-Hermitian $K$ is unitary; $S=|U|^{\odot2}$ is the doubly stochastic energy
bookkeeping, while the heterodyne readout $W$ is the signed mixing applied to
streams.}
\label{fig:schematic}
\end{figure}
...
construction, and the operator costs one $n\times n$ complex linear solve; the
Cayley map is a diffeomorphism onto a dense open subset of $\mathrm{U}(n)$, so
the parameterization is lossless and smooth, and gradients (through
\texttt{torch.linalg.solve} in \texttt{complex64}) were finite in all experiments.

\begin{proposition}[Non-expansive heterodyne mixing]
\label{prop:safety}
Writing $U_{jk} = |U_{jk}|e^{i\theta_{jk}}$, we have
$W_{jk} = |U_{jk}|\cos(\theta_{jk}+\phi_k)$, so $|W_{jk}| \le |U_{jk}|$ and
$\sum_{k} W_{jk}^{2} \le \sum_{k} |U_{jk}|^{2} = 1$ for every row, whence
$\|Wx\|_2 \le \|x\|_2$: mixing never amplifies energy, while individual weights
become negative whenever a phase shifts by $\pi$ (destructive interference).
\end{proposition}
...
$a=\tan\tfrac{\pi}{4}$, $B=0$; the Cayley transform of a rotation generator is the
rotation with $\tan\tfrac{\theta}{2}=a$, i.e.\ $\theta=\tfrac{\pi}{2}$, and row~0
of $U = R(\theta)\oplus I_{n-2}$ is $\tfrac{1}{\sqrt2}(1,1,0,\dots,0)$ (sign of
$a$ chosen accordingly). Set $\phi = (0,\pi,0,\dots,0)$. Then
...
entrywise non-negative mixing row, including Sinkhorn/Birkhoff projections and
orthostochastic readouts $Q\odot Q$---,
\begin{equation}
...
budget unistochastic while releasing the sign of the mixing---probabilities are
additive and non-negative, whereas amplitudes interfere. An inverse-free special
case replaces $U$ by $\tfrac{n(n-1)}{2}$ Givens rotations $Q$ ($S=Q\odot Q$), a
$O(n^2)$-parameter unistochastic subfamily (Sec.~\ref{sec:exp-ops}).
...
\begin{figure}[t]
\centering
\includegraphics[width=0.66\linewidth]{figs/fig_disentanglement.png}
\caption{\textbf{Subtractive disentanglement} ($Y = X_0 - X_1$). \textbf{(a)}
Uni-mHC and GivensMHC reach machine precision; non-negative baselines plateau at
the impossibility floor (Prop.~\ref{prop:floor}). \textbf{(b)} Learned
coefficients $\gamma W_{0k}$: phase interference yields the exact $(+1,-1,0,0)$
pattern; non-negative mixers degenerate to a pure copy $(+1,0,0,0)$.}
\label{fig:disent}
\end{figure}
...
interference pattern of Theorem~\ref{thm:subtraction}. Sinkhorn mHC and the
orthostochastic baseline stall at $\SKToyLoss$ and $\OrthoToyLoss$ (floor: $1.0$),
their best solution being the trivial copy $X_0$. All gradients were finite
throughout, confirming stable complex autograd.
...
At this scale the vanilla GPT overfits within the short horizon (train $0.11$,
val $3.46$), while every manifold-constrained variant---whose identity-initialized
stream topology warms up effective capacity gradually---holds val $\approx 1.51$.
...
interference---impossible for entrywise non-negative mixers, as we prove, and
verified empirically ($10^{-11}$ vs.\ $0.99$). Future work: token-conditional
phases, scaling studies, and links to quantum circuit compilation.

\begin{thebibliography}{9}

\bibitem[DeepSeek(2025)]{deepseek2025mhc}
DeepSeek. Manifold-constrained hyper-connections. Technical report, December 2025.
...
\bibitem[Lu et~al.(2025)]{lu2025hyperconnections}
Z.~Lu, K.~Yang, X.~Zang, Y.~Dang, D.~Huang, C.~Wang, and Y.~Dong.
Hyper-connections. \emph{arXiv preprint arXiv:2509.04798}, 2025.

\bibitem[sHC(2026)]{shc2026}
sHC authors.
Signed hyper-connections: On the additive limitation of doubly stochastic stream
mixing. \emph{arXiv preprint arXiv:2603.20896}, 2026.
