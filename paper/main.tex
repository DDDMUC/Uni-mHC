...
spectral-sphere-constrained hyper-connections (sHC). We propose \textbf{Uni-mHC},
lifting the constraint manifold from the Birkhoff polytope to the
...
$W=\operatorname{Re}(Ue^{i\phi})$ admits negative weights through destructive
(phase-$\pi$) interference while staying row-wise non-expansive.
...
\caption{\small\textbf{Subtractive disentanglement} ($Y = X_0 - X_1$). \textbf{(a)}
...
Vanilla timing omitted (stock timer mixes training with eval pauses;
speed claim rests on Table~\ref{tab:ops}).}
\label{tab:gpt}
\small
\begin{tabular}{lccc}
\toprule
...
At this scale vanilla GPT overfits (train $0.11$, val $3.46$), while constrained
variants hold val $\approx 1.51$ (identity-initialized warm-up).
...
interference---unmatchable by any single non-negative linear mixing layer in one
forward pass (Prop.~\ref{prop:floor}), as verified ($10^{-11}$ vs.\ $0.99$).
Future work: token-conditional phases and scaling studies.
